...
from scipy.stats import invgauss, t, norm
...
def alphaMatrix(df, reverse_dict, issuers, factors, issuer_factors):
    index = df.index
    # issuers_catagorized = analyseIssuer(issuers, factors)
    issuers_catagorized = issuer_factors
    alpha_vectors = []
...
def cholesky(ticker_dict, issuers, factors, issuer_factors):
    ...
    alpha_vectors, alpha_dropped = alphaMatrix(df, reverse_dict, issuers, factors, issuer_factors)
    # print(alpha_vectors)
    # print(alpha_dropped)
...
def MC_sim(issuers, alpha, beta, threshhold, chol, cop, df):
    ...
    for i in range(1000000):
        ...
        # box_muller1 = [np.sqrt(-2*np.log(x))*np.cos(2*np.pi*y) for x,y in zip(random_F1, random_F2)]
        random_chi = np.random.chisquare(df)
        W = np.sqrt(df/random_chi)
        # random_F = norm.ppf(random_F1)*W
        random_F = norm.ppf(random_F1)
        # random_F = np.array(box_muller1)*W
        ...
        loss_list.append(L)
    # fig, ax = plt.subplots()
    # ax.hist(X_list, bins=50)
    # plt.show()
    return loss_list


def simulateCorrelation(issuers, alpha, beta, threshhold, chol, cop, df):

    loss_list = []

    with concurrent.futures.ProcessPoolExecutor() as executor:
        values = [executor.submit(MC_sim, issuers, alpha, beta, threshhold, chol, cop, df) for _ in range(10)]
